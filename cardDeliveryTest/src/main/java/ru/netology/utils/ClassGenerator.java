...
public class ClassGenerator {
	...
	private static final String GENERATED_DATA_FILES_PREFIX = "GeneratedTestFile_";
	...
	private static final int NUMBER_OF_TEST_FILES_TO_GENERATE = 50;
		...
		try {
			...
			for (int i = 0; i < NUMBER_OF_TEST_FILES_TO_GENERATE; i++) {
				...
				try (Writer writer = Files.newBufferedWriter(generatedFilePath)) {
					...
					gson.toJson(FakerClass.getInstance().createPerson(), writer);
				} catch (IOException e) {
					throw new RuntimeException(String.format(
